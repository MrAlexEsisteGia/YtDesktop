...
function makeClient(clientId) {
    ...
    rpc.login({clientId})
        .then(() => {
            instance.emit('connected', rpc.user);//the smol modification to callback the user data
            connected = true;

            //debug dont leave this in dumbass
            //rpc.getRelationships().then(sas => {console.log(sas)})
